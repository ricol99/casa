...
function LightwaveRF(config) {
	...
	this.receiveSocket.on("message", function (message, rinfo) {
		...
		var content = parts.join(",").replace(/(\r\n|\n|\r)/gm,"");
                ...
                this.completeCallback(code.toString(), undefined, content);
		...
		this.responseListeners[parseInt(code).toString()] = {
			...
			listener: function(returnedCode, content) {
				callback(undefined, content);
                        ...
                        timeout: setTimeout(function(_that, _code) {
                        	_that.completeCallback(_code, {message: 'request timed out' }, undefined, true);
			}, 5000, this, parseInt(code).toString());
		}
	}
...
LightwaveRF.prototype.completeCallback = function(_code, _error, _content, _timedOut) {
	//Check to see if we have a relevant listener
	var responseListenerData = this.responseListeners[_code];
	if (responseListenerData) {
                if (!_timedOut) {
			clearTimeout(responseListenerData.timeout);
                }
		//Call the response listener
		responseListenerData.listener(_code,_content);
		delete this.responseListeners[_code];
		this.responseListeners[_code] = undefined;
	}
}
