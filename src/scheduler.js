...
var Thing = require('./propertybinder');
      ...
      if (_this.enabled) {
        var difference = Math.abs(_this.endValue - _this.value);
         ...
         if (difference <= Math.abs(_this.step)) {
            _this.thing.updateProperty(_this.propName, _this.endValue, { sourceName: _this.thing.uName });
         }
         else {
            _this.value += _this.step;
            _this.thing.updateProperty(_this.propName, _this.floorOutput(_this.value), { sourceName: _this.thing.uName });
            _this.nextInterval();
         }
      }
...
function resetJob(_this, _event) {
   ...
   _event.job = schedule.scheduleJob(_event.rule, function() {
      ...
      if (_event.ramp == undefined) {
         that.updateProperty(_event.name, _event.propertyValue , { sourceName: that.uName });
      }
      else {
...
function createNewRamp(_this, _event) {
   ...
   if (_event.ramp.startValue != undefined) {
      _this.updateProperty(_event.name, _event.ramp.startValue , { sourceName: _this.uName });
   }
...
Scheduler.prototype.coldStart = function(_event) {
   var that = this;
   ...
   scheduleAllJobs(this, function() {
      ...
      that.updateProperty(closestEvent.name, value, { sourceName: that.uName, coldStart: true });
   });
}
